...
### Phase 4.6: UI Polish & New Calculations
- [x] **Task 4.6.1: Interactive Probability Widget Dashbaord**: Add the interactive probability Widge Dashboard to matchday analytics. The interactive probability dashboard should be very similar to the probability widget found on the Head to Head tab, but with the matchday filters from Matchday Deep Dive.  
- [ ] **Task 4.6.1.2: Interactive Probability Widget Dashboard (Widgets)**: Let's remove the colors from the percentages in each widget in the interactive probability widge dashboard all of the probabilities and percentages should remain the same simple color. 
- [ ] **Task 4.6.1.3: Interactive Probability Widget Dashboard (Widgets)**: For Anything that is tied to "Home" or "Away" We actually want to switch this out to the team's name instead of home, and the away team's name instead of away. We need to be particularly cautious when solving for this because if the user changes matchup context from exact to overall the names will flip flop. 
- [ ] **Task 4.6.1.4: Interactive Probability Widget Dashboard (Matchup Context)**: Also let's switch out anything that is "Total Goals = " we don't want equals, we want "Total Goals" > than, and this is the same concept for Total Team Goals, instead of home team goals = 1 we would want "team name that played home" > than 1, instead of away team goals = 1 we would want "team name that played away" > than 1. In general it should be "team name that played home/away" > than X, and X, and we want widgets for X = 1, 2, 3, and 4. 
